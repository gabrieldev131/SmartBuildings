...
class GlobalIdentity:
    ...
    Agora inclui histórico de Bounding Boxes e Adaptação Visual (EMA).
    """
    def __init__(self, global_id: int, initial_feature: np.ndarray, bbox: list, initial_cam_id: str, start_time: float):
        ...
        self.last_bbox = bbox  # NOVO: Guarda a última posição conhecida
    ...
    def update(self, new_feature_vector: np.ndarray, bbox: list, cam_id: str, current_time: float) -> bool:
        """
        Atualiza a posição, aparência e gere o histórico.
        Retorna True se houve uma mudança de câmara (Transição), False caso contrário.
        """
        # NOVO: Exponential Moving Average (EMA) - Adapta-se suavemente a mudanças de luz/ângulo
        self.feature_vector = 0.8 * self.feature_vector + 0.2 * new_feature_vector
        self.last_bbox = bbox
    ...
    # -------------------------------------------------------------------------
    # MÉTODOS DE EXPORTAÇÃO PARA ESTATÍSTICA
    # -------------------------------------------------------------------------
    ...
    def get_raw_history(self) -> list[dict]:
        ...
        for record in self.camera_history:
            ...
            # Se a pessoa ainda está na câmara no momento da exportação, fechamos com o tempo atual
            if t_out is None:
                t_out = time.time()
